...
headerRegistratio.addEventListener('click', () => {
 ...
 document.addEventListener('DOMContentLoaded', () => {
  const questItems = document.querySelectorAll('.home__quest-item');

  questItems.forEach(item => {
    const button = item.querySelector('.home__quest-question');

    button.addEventListener('click', () => {
      const isActive = item.classList.contains('active');

      // Закриваємо всі інші відкриті питання (опціонально)
      questItems.forEach(otherItem => {
        if (otherItem !== item) {
          otherItem.classList.remove('active');
        }
      });

      // Перемикаємо стан поточного
